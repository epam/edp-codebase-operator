...
func TestSetIntermediateSuccessFieldsProjectGerrit(t *testing.T) {
	...
	handler := PutProjectGerrit{clientSet: cs}
...
func TestFailedStatus(t *testing.T) {
	...
	setFailedFields(&cr, v1alpha1.CleanData, "some error")

	assert.Equal(t, v1alpha1.CleanData, cr.Status.Action)
	assert.Equal(t, "some error", cr.Status.DetailedMessage)
	assert.Equal(t, v1alpha1.Error, cr.Status.Result)
